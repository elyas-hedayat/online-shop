...
class UserPermission(BasePermission):
    ...
    def has_permission(self, request):
        return not BlockedUser.objects.is_user_blocked(
            request.user.id, request.POST.get("email")
        )
...
class ObjectLevelMixin(BaseCommentPermission):
    ...
    def get_object(self):
        raise ImproperlyConfigured(
            ErrorMessage.METHOD_NOT_IMPLEMENTED.format(
                class_name=self.__class__.__name__, method_name="get_object()"
            )
        )
...
class CanDeleteMixin(ObjectLevelMixin, ValidatorMixin, abc.ABC):
    def has_object_permission(self, request, obj):
        return bool(
            ...
            or is_comment_admin(request.user)
