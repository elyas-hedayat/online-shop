...
class ReactionInstanceManager(models.Manager):
    def clean_reaction_type(self, reaction_type):
        ...
        raise ValidationError(
            ReactionError.TYPE_INVALID.format(reaction_type=reaction_type),
            code="invalid",
        )
    ...
    def set_reaction(self, user, reaction, reaction_type):
        ...
        try:
            ...
            instance = self.create(
                reaction=reaction, user=user, reaction_type=reaction_type
            )
            created = True

        if not created:
            self._delete_and_create_new_instance(
                instance=instance, user=user, reaction_type=reaction_type
            )
