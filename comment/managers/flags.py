...
class FlagInstanceManager(models.Manager):
    ...
    def _clean_reason(self, reason):
        err = ValidationError(
            FlagError.REASON_INVALID.format(reason=reason), code="invalid"
        )
        try:
            reason = int(reason)
    ...
    def create_flag(self, user, flag, reason, info):
        err = ValidationError(
            FlagError.ALREADY_FLAGGED_BY_USER.format(user=user), code="invalid"
        )
    ...
    def delete_flag(self, user, flag):
        err = ValidationError(
            FlagError.NOT_FLAGGED_BY_USER.format(user=user), code="invalid"
        )
        try:
