...
from comment.managers import FlagInstanceManager, FlagManager
...
class Flag(models.Model):
    ...
    def get_clean_state(self, state):
        err = ValidationError(
            FlagError.STATE_INVALID.format(state=state), code="invalid"
        )
        try:
            state = int(state)
    ...
    def toggle_flagged_state(self):
        ...
        if not allowed_flags:
            return
        self.refresh_from_db()
        if self.count > allowed_flags and self.state not in [
            self.RESOLVED,
            self.REJECTED,
        ]:
            self.state = self.FLAGGED
...
class FlagInstance(models.Model):
    ...
    reason = models.SmallIntegerField(
        choices=objects.REASONS, default=objects.reason_values[0]
    )
