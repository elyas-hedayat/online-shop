...
class BaseToggleFollowView(ContentTypeValidator, DABResponseData):
    ...
    def get_response_class(self):
        assert self.response_class is not None, (
            ...
            "or override the `get_response_class()` method." % self.__class__.__name__
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if email and not DABEmailValidator(email).is_valid():
            self.error = {"invalid_email": DABEmailValidator.message}
            self.status = 400
        ...
        if not email:
            self.error = {
                "email_required": FollowError.EMAIL_REQUIRED.format(
        ...
        following = Follower.objects.toggle_follow(
            email=email, model_object=self.model_obj, username=username
        )
        self.data = {
            "following": following,
