...
class BaseToggleBlockingView(DABResponseData):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            self.error = {"detail": BlockUserError.INVALID}
            self.status = 400
        ...
        (
            blocked_user,
            created,
        ) = BlockedUser.objects.get_or_create_blocked_user_for_comment(comment)
        ...
        BlockedUserHistory.objects.create_history(
            blocked_user=blocked_user, blocker=request.user, reason=reason
        )
        self.data = {
