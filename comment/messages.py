...
class ErrorMessage:
    ...
    METHOD_NOT_IMPLEMENTED = _(
        "Your {class_name} class has not defined a {method_name} method, which is required."
    )
    ...
    WRAP_CONTENT_WORDS_NOT_INT = _(
        "Comment App: settings var COMMENT_WRAP_CONTENT_WORDS must be an integer"
    )
...
class ContentTypeError:
    ...
    MODEL_ID_INVALID = _(
        "{model_id} is NOT a valid model id for the model {model_name}"
    )
    PARENT_ID_INVALID = _(
        "{parent_id} is NOT a valid id for a parent comment or "
...
class ReactionError:
    TYPE_INVALID = _(
        "Reaction must be an valid ReactionManager.RelationType. {reaction_type} is not"
    )


class EmailError:
    EMAIL_INVALID = _("Enter a valid email address.")
    EMAIL_REQUIRED_FOR_ANONYMOUS = _(
        "Email is required for posting anonymous comments."
    )
