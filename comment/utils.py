...
def get_profile_content_type():
    ...
    try:
        content_type = ContentType.objects.get(
            app_label=profile_app_name, model=profile_model_name.lower()
        )
...
def can_moderate_flagging(user):
    return bool(
        settings.COMMENT_FLAGS_ALLOWED
        and (is_comment_admin(user) or is_comment_moderator(user))
    )


def can_moderator_block_users(moderator):
    return settings.COMMENT_ALLOW_MODERATOR_TO_BLOCK and is_comment_moderator(moderator)


def can_block_user(user):
    return bool(
        settings.COMMENT_ALLOW_BLOCKING_USERS
        and (is_comment_admin(user) or can_moderator_block_users(user))
    )
        ...
        comment_dict.update(
            {
                ...
                "parent": comment_model.objects.get_parent_comment(
                    comment_dict["parent"]
                ),
            }
        )
