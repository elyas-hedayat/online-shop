...
from comment.messages import ContentTypeError, EmailError, ExceptionError
...
class BaseValidatorMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {"type": ExceptionError.ERROR_TYPE, "detail": exc.detail}, status=400
            )
        return super().dispatch(request, *args, **kwargs)
...
class ContentTypeValidator(BaseValidatorMixin):
    ...
    def validate_content_type_object(self, app_name, model_name):
        try:
            ...
            self.error = ContentTypeError.MODEL_NAME_INVALID.format(
    ...
    def validate_model_id(self, model_id):
        ...
        try:
            ...
            self.error = ContentTypeError.ID_NOT_INTEGER.format(
                var_name="model", id=model_id
            )
            raise CommentBadRequest(self.error)
        return model_id
    ...
    def validate_model_object(self, app_name, model_name, model_id):
        ...
        if not model_query.exists() and model_query.count() != 1:
            self.error = ContentTypeError.MODEL_ID_INVALID.format(
    ...
    def validate(self, request):
        ...
        self.model_obj = self.validate_model_object(
            validated_app_name, validated_model_name, validated_model_id
        )
...
class ParentIdValidator(BaseValidatorMixin):
    ...
    def validate(self, request):
        ...
        self.parent_comment = self.validate_comment_object(
            model_id, validated_parent_id
        )
