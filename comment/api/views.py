...
from comment.api.permissions import (
    CanBlockUsers,
    CanChangeFlaggedCommentState,
    CanCreatePermission,
    CanGetSubscribers,
    FlagEnabledPermission,
    IsOwnerOrReadOnly,
    SubscriptionEnabled,
    UserPermittedOrReadOnly,
)
from comment.api.serializers import CommentCreateSerializer, CommentSerializer
from comment.messages import EmailError, FlagError
...
from comment.utils import CommentFailReason, get_comment_from_key
from comment.validators import ContentTypeValidator, ValidatorMixin
...
class CommentList(ContentTypeValidator, generics.ListAPIView):
    ...
    def get_queryset(self):
        self.validate(self.request)
        return Comment.objects.filter_parents_by_object(self.model_obj).select_related(
            "user", "reaction", "flag"
        )


class CommentDetail(generics.RetrieveUpdateDestroyAPIView):
    queryset = Comment.objects.filter(accepted=True).select_related(
        "user", "reaction", "flag"
    )
    serializer_class = CommentSerializer
    permission_classes = (
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ReactionInstance.objects.set_reaction(
                user=request.user, reaction=reaction_obj, reaction_type=reaction_type
            )
...
class ConfirmComment(APIView, CommentCreateMixin):
    def get(self, request, *args, **kwargs):
        ...
        if temp_comment.why_invalid == CommentFailReason.BAD:
            return Response(
                {"detail": EmailError.BROKEN_VERIFICATION_LINK},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if temp_comment.why_invalid == CommentFailReason.EXISTS:
            return Response(
                {"detail": EmailError.USED_VERIFICATION_LINK}, status=status.HTTP_200_OK
            )
        ...
        return Response(
            {
                ...
                "followers": Follower.objects.get_emails_for_model_object(
                    self.model_obj
                ),
            }
        )
