...
from comment.utils import get_profile_instance, get_user_for_request
    ...
    def validate_email(email):
        if not email:
            raise serializers.ValidationError(
                detail={"email": [EmailError.EMAIL_REQUIRED_FOR_ANONYMOUS]},
                code="required",
            )
...
class FlagSerializer(serializers.ModelSerializer):
    ...
    def get_reporters(obj):
        return [
            {"id": flag_instance.user.id, "username": flag_instance.user.USERNAME_FIELD}
