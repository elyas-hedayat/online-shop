...
urlpatterns = [
    ...
    re_path(
        r"^confirm/(?P<key>[^/]+)/$", ConfirmComment.as_view(), name="confirm-comment"
    ),
    path(
        "toggle-subscription/", ToggleFollowView.as_view(), name="toggle-subscription"
    ),
    ...
    path(
        "jsi18n/",
        cache_page(86400, key_prefix="js18n-%s" % __version__)(
