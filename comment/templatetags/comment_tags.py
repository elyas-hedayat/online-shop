...
from comment.models import BlockedUser, FlagInstance, Follower, ReactionInstance
...
def get_comments_count(obj, user):
    return obj.comments.all_comments_by_object(
        obj, include_flagged=is_comment_moderator(user)
    ).count()
...
def has_reacted(comment, user, reaction):
    ...
    if user.is_authenticated:
        ...
        return ReactionInstance.objects.filter(
            user=user, reaction_type=reaction_type.value, reaction__comment=comment
        ).exists()
