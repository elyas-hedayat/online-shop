...
class GroupsAndPermissionsTest(TestCase):
    ...
    def _get_groups_not_created(self):
        return set(self.groups) - set(
            Group.objects.filter(name__in=self.groups).values_list("name", flat=True)
        )

    def _get_permissions_not_created(self):
        return set(self.permissions) - set(
            Permission.objects.filter(codename__in=self.permissions).values_list(
                "codename", flat=True
            )
        )
