...
class TestPostSave(BaseCommentSignalTest):
    ...
    def test_increase_flag_count(self):
        ...
        flag_instance = FlagInstance.objects.get(
            user=self.user, flag__comment=self.comment
        )
...
class TestPostDelete(BaseCommentSignalTest):
    def test_reaction_decrease_count(self):
        ...
        comment = self.comment
        instance = self.create_reaction_instance(
            self.user, self.comment, self.LIKE.name
        )
