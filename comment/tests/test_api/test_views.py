...
class BaseAPIViewTest(BaseAPITest):
    ...
    def comment_count_test(self):
        self.assertEqual(
            Comment.objects.filter_parents_by_object(self.post_1).count(), self.parents
        )
...
class CommentListTest(BaseAPIViewTest):
    ...
    def test_retrieving_with_invalid_app_name(self):
        ...
        self.assertEqual(
            ...
            ContentTypeError.APP_NAME_INVALID.format(app_name=app_name),
        )
        self.assertTextTranslated(response.data["detail"], url)
    ...
    def test_retrieving_with_invalid_model_name(self):
        ...
        self.assertEqual(
            response.data["detail"],
            ContentTypeError.MODEL_NAME_INVALID.format(model_name=model_name),
        )
        self.assertTextTranslated(response.data["detail"], url)
    ...
    def test_retrieving_with_non_exitsting_model_id(self):
        ...
        self.assertEqual(
            response.data["detail"],
            ContentTypeError.MODEL_ID_INVALID.format(
                model_id=model_id, model_name=url_data["model_name"]
            ),
        )
        self.assertTextTranslated(response.data["detail"], url)
...
class CommentCreateTest(BaseAPIViewTest):
    ...
    def test_create_parent_comment(self):
        ...
        response = self.client.post(
            self.get_url(self.get_base_url(), **url_data), data=data
        )
        ...
        self.assertEqual(
            Comment.objects.get(id=comment_id).email, response.wsgi_request.user.email
        )
    ...
    def test_create_child_comment(self):
        ...
        url_data["parent_id"] = (
            Comment.objects.all_comments_by_object(model_obj)
            .filter(parent=None)
            .first()
            .id
        )
    ...
    def test_for_anonymous_with_invalid_data(self):
        ...
        self.assertEqual(
            response.json()["email"],
            {"email": [EmailError.EMAIL_REQUIRED_FOR_ANONYMOUS]},
        )
    ...
    def test_parent_id_does_not_exist(self):
        ...
        self.assertEqual(
            response.data["detail"],
            ContentTypeError.PARENT_ID_INVALID.format(parent_id=parent_id),
        )
        self.assertTextTranslated(response.data["detail"], base_url)
    ...
    def test_parent_id_does_not_belong_to_model_object(self):
        ...
        url_data = self.url_data.copy()
        parent_id = 1
        url_data.update({"parent_id": parent_id, "model_id": 2})
        data = {"content": "new child comment from api"}
...
class CommentDetailTest(BaseAPIViewTest):
    ...
    def test_retrieval(self):
        ...
        self.assertEqual(
            response.data["content"], Comment.objects.get(id=self.c_id).content
        )
    ...
    def test_update(self):
        ...
        response = self.client.put(
            self.get_base_url(), data=data, content_type="application/json"
        )
    ...
    def test_delete_parent(self):
        ...
        self.assertEqual(
            Comment.objects.all().count() + 1 + reply_count, self.all_comments
        )
...
class CommentDetailForReactionTest(BaseAPIViewTest):
    ...
    def test_cannot_update_comment_content(self):
        ...
        response = self.client.post(
            self.get_base_url(self.like), data=data, content_type="application/json"
        )
    ...
    def test_invalid_reaction_type(self):
        ...
        self.assertEqual(
            error, ReactionError.TYPE_INVALID.format(reaction_type=reaction)
        )
...
class APIConfirmCommentViewTest(BaseAnonymousCommentTest, BaseAPITest):
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.comment = cls.create_anonymous_comment(
            posted=timezone.now(), email="a@a.com"
        )
    ...
    def test_comment_exists(self):
        ...
        comment_dict.update(
            {"posted": str(self.comment.posted), "email": self.comment.email}
        )
    ...
    def test_success_without_notification(self):
        ...
        comment = Comment.objects.get(
            email=self.comment_obj.email, posted=self.time_posted
        )
...
class APIGetSubscribersTest(BaseAPITest):
    ...
    def test_only_moderators_can_get_followers(self):
        ...
        self.assertEqual(
            list(response.data["followers"]), [self.comment_get_followers.email]
        )
