...
class APICommentSerializersTest(BaseAPITest):
    def setUp(self):
        super().setUp()
        self.parent_count = Comment.objects.filter_parents_by_object(
            self.post_1
        ).count()
    ...
    def comment_count_test(self):
        self.assertEqual(
            Comment.objects.filter_parents_by_object(self.post_1).count(),
            self.parent_count,
        )
    ...
    def test_create_parent_comment_serializer(self):
        ...
        data = {"model_obj": self.post_1, "parent_comment": None, "request": request}
    ...
    def test_create_comment_serializer_for_anonymous(self):
        ...
        comment = serializer.create(
            validated_data={"content": "anonymous posting", "email": "abc@abc.com"}
        )
    ...
    def test_create_comment_serializer_for_anonymous_missing_email(self):
        ...
        self.assertEqual(
            e.exception.detail, {"email": [EmailError.EMAIL_REQUIRED_FOR_ANONYMOUS]}
        )
    ...
    def test_passing_context_to_serializer(self):
        ...
        serializer = CommentSerializer(
            self.comment_1, context={"reaction_update": True}
        )
