...
from comment.mixins import (
    AJAXRequiredMixin,
    BaseCommentPermission,
    BaseCreatePermission,
    BasePermission,
    ObjectLevelMixin,
)
from comment.tests.base import BaseCommentFlagTest, BaseCommentMixinTest
...
class AJAXMixinTest(BaseCommentMixinTest):
    ...
    def test_non_ajax_request(self):
        ...
        self.assert_permission_denied_response(
            response, reason=ErrorMessage.NON_AJAX_REQUEST
        )
...
class BasePermissionTest(BaseCommentMixinTest):
    ...
    def test_dispatch_with_no_permission(self, _):
        ...
        self.assert_permission_denied_response(
            response, reason=ErrorMessage.NOT_AUTHORIZED
        )
...
class CanCreateMixinTest(BaseCommentMixinTest):
    ...
    @patch.object(settings, "COMMENT_ALLOW_ANONYMOUS", False)
    def test_logged_out_user_permission(
        self,
    ):
        self.client.logout()
        ...
        self.assert_permission_denied_response(
            response, reason=ErrorMessage.LOGIN_REQUIRED
        )
...
class ObjectLevelMixinTest(BaseCommentMixinTest):
    ...
    def test_has_object_permission(self):
        self.assertTrue(
            self.object_mixin.has_object_permission(self.request, self.comment)
        )
...
class BaseFlagMixinTest(BaseCommentFlagTest):
    ...
    def test_flag_not_enabled_permission(self):
        ...
        self.assertEqual(response.status_code, 403)
        self.assert_permission_denied_response(
            response, reason=FlagError.SYSTEM_NOT_ENABLED
        )
...
class CanSubscribeMixinTest(BaseCommentMixinTest):
    def setUp(self):
        ...
        self.toggle_follow_url = self.get_url(
            reverse("comment:toggle-subscription"),
            app_name="comment",
            model_name="comment",
            model_id=self.comment.id,
        )
    ...
    def test_user_cannot_subscribe(self):
        ...
        self.assertEqual(response.status_code, 403)
        self.assert_permission_denied_response(
            response, reason=FollowError.SYSTEM_NOT_ENABLED
        )
