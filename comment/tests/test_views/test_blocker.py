...
class BaseToggleBlockingViewTest(BaseCommentMixinTest):
    ...
    def setUpTestData(cls):
        ...
        cls.anonymous_comment_for_blocking = cls.create_anonymous_comment(
    ...
    def test_block_comment_user_by_passing_non_int_comment_id(self):
        ...
        response = self.client.post(
            self.toggle_blocking_url, data=data, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_block_comment_user_with_missing_comment_id(self):
        response = self.client.post(
            self.toggle_blocking_url, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_block_comment_user(self):
        self.assertFalse(
            BlockedUser.objects.is_user_blocked(
                user_id=self.comment_for_blocking.user.id
            )
        )
        ...
        self.assertEqual(
            response.json()["data"]["blocked_user"],
            self.comment_for_blocking.get_username(),
        )
        self.assertTrue(response.json()["data"]["blocked"])
        self.assertEqual(
            response.json()["data"]["urlhash"], self.comment_for_blocking.urlhash
        )
    ...
    def test_block_anonymous_comment_email(self):
        self.assertFalse(
            BlockedUser.objects.is_user_blocked(
                email=self.anonymous_comment_for_blocking.email
            )
        )
        ...
        self.assertEqual(
            response.json()["data"]["blocked_user"],
            self.anonymous_comment_for_blocking.get_username(),
        )
        self.assertTrue(response.json()["data"]["blocked"])
        self.assertEqual(
            response.json()["data"]["urlhash"],
            self.anonymous_comment_for_blocking.urlhash,
        )
