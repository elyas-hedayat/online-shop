...
class CommentViewTestCase(BaseCommentViewTest):
    ...
    def test_create_child_comment(self):
        ...
        child_comment = Comment.objects.get(
            object_id=self.post_1.id, parent=parent_comment
        )
...
class TestEditComment(BaseCommentViewTest):
    ...
    def test_edit_comment(self):
        ...
        response = self.client.post(
            post_url, data=data, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        ...
        with self.assertRaises(ValueError) as error:
            self.client.post(
                self.get_url("comment:edit", comment.id),
                data=data,
...
class TestDeleteComment(BaseCommentViewTest):
    ...
    def test_delete_comment_by_moderator(self):
        ...
        response = self.client.post(
            self.get_url("comment:delete", comment.id), data=self.data
        )
...
class ConfirmCommentViewTest(BaseAnonymousCommentTest):
    ...
    def test_comment_exists(self):
        ...
        comment_dict.update({"posted": str(comment.posted), "email": comment.email})
