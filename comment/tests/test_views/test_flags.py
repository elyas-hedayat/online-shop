...
from comment.messages import FlagError, FlagInfo
...
class ChangeFlagStateViewTest(BaseCommentFlagTest):
    ...
    def setUp(self):
        ...
        self.create_flag_instance(
            self.user_1, self.comment_for_change_state, **self.flag_data
        )
        self.create_flag_instance(
            self.user_2, self.comment_for_change_state, **self.flag_data
        )
    ...
    def test_change_flag_state_for_unflagged_comment(self):
        ...
        response = self.client.post(
            self.get_url("comment:flag-change-state", comment.id), data=self.data
        )
    ...
    def test_change_flag_state_by_not_permitted_user(self):
        ...
        response = self.client.post(
            self.get_url("comment:flag-change-state", self.comment_for_change_state.id),
            data=self.data,
        )
    ...
    def test_change_flag_state_with_wrong_state_value(self):
        ...
        self.assertEqual(int(self.client.session["_auth_user_id"]), self.moderator.id)
        self.assertEqual(
            self.comment_for_change_state.flag.state,
            self.comment_for_change_state.flag.FLAGGED,
        )
    ...
    def test_change_flag_state_success(self):
        ...
        self.assertEqual(
            ...
            self.comment_for_change_state.flag.REJECTED,
        )
