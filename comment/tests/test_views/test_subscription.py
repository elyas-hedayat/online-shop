...
from comment.messages import EmailError, FollowError
...
class BaseToggleFollowViewTest(BaseCommentMixinTest):
    ...
    def test_email_required_to_follow_object(self):
        ...
        response = self.client.post(
            self.toggle_follow_url, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.json()["error"]["email_required"],
            FollowError.EMAIL_REQUIRED.format(model_object=self.comment),
        )
    ...
    def test_invalid_email(self):
        ...
        response = self.client.post(
            self.toggle_follow_url, HTTP_X_REQUESTED_WITH="XMLHttpRequest", data=data
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.json()["error"]["invalid_email"], EmailError.EMAIL_INVALID
        )
