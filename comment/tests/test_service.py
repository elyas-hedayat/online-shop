...
class TestDABEmailService(BaseAnonymousCommentTest):
    def setUp(self):
        ...
        self.confirmation_url_drf = reverse(
            "comment-api:confirm-comment", args=[self.key]
        )
    ...
    def setUpTestData(cls):
        super().setUpTestData()
        cls.child_comment = cls.create_comment(
            cls.content_object_1, parent=cls.comment_1
        )

    def email_contents_test(self, contents, *args):
        for arg in args:
            self.assertIs(
                arg in contents, True, msg=f"{arg} not present in email contents"
            )
    ...
    def test_get_message_for_html(self):
        ...
        msg = self.email_service.get_message(
            subject, body, self.receivers, html_msg=html
        )
        self.assertIsInstance(msg, EmailMultiAlternatives)
        self.assertEqual(msg.to, self.receivers)
    ...
    def test_get_message_templates_text_only(self):
        ...
        text_msg, html_msg = email_service.get_message_templates(
            text_template, html_template, context
        )
    ...
    def test_get_message_templates_with_html(self):
        ...
        text_template = "comment/anonymous/confirmation_request.txt"
        html_template = "comment/anonymous/confirmation_request.html"
        text_msg, html_msg = self.email_service.get_message_templates(
            text_template, html_template, context
        )
    ...
    def test_send_messages(self):
        ...
        messages = [
            self.email_service.get_message(
                "test subject", "test body", ["test@test"], html_msg="test msg"
            )
            for _ in range(100)
        ]
    ...
    def test_get_thread_for_parent_comment(self):
        ...
        self.assertIsInstance(
            thread, self.email_service.comment.content_object.__class__
        )
    ...
    def test_send_notification_to_followers_return_none(self):
        ...
        followers = Follower.objects.filter_for_model_object(
            self.comment_obj.content_object
        ).exclude(email=self.comment_obj.email)
    ...
    def test_send_notification_to_followers(self):
        Follower.objects.follow(
            "any@test.com", "test_user", self.comment_obj.content_object
        )
        followers = Follower.objects.filter_for_model_object(
            self.comment_obj.content_object
        ).exclude(email=self.comment_obj.email)
        ...
        self.assertEqual(
            sent_email.subject,
            EmailInfo.NOTIFICATION_SUBJECT.format(
                username=username, thread_name=thread_name
            ),
        )
