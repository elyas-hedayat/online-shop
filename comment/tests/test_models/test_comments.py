...
class CommentModelTest(BaseCommentManagerTest):
    ...
    def test_parent_comment_properties(self):
        ...
        self.assertEqual(
            str(comment), f"comment by {comment.user}: {comment.content[:20]}"
        )
        self.assertEqual(
            repr(comment), f"comment by {comment.user}: {comment.content[:20]}"
        )
    ...
    def test_child_comment_properties(self):
        ...
        self.assertIsNotNone(comment)
        self.assertEqual(
            str(comment), f"reply by {comment.user}: {comment.content[:20]}"
        )
        self.assertEqual(
            repr(comment), f"reply by {comment.user}: {comment.content[:20]}"
        )
    ...
    def test_is_edited_for_anonymous_comment(self):
        comment = self.create_anonymous_comment(
            posted=timezone.now() - timezone.timedelta(days=1)
        )
    ...
    def test_replies_method_without_any_flags(self):
        ...
        self.assertEqual(
            self.parent_comment_2.replies(include_flagged=True).count(), init_count
        )
    ...
    def test_urlhash_is_unique(self, mocked_generate_urlhash):
        mocked_generate_urlhash.side_effect = [
            "first_urlhash",
            "first_urlhash",
            "second_urlhash",
        ]
    ...
    def test_get_username_for_anonymous_comment(self):
        ...
        with patch.object(settings, "COMMENT_USE_EMAIL_FIRST_PART_AS_USERNAME", False):
            self.assertEqual(
                comment.get_username(), settings.COMMENT_ANONYMOUS_USERNAME
            )
...
class CommentModelManagerTest(BaseCommentManagerTest):
    ...
    def test_filtering_flagged_comment(self):
        ...
        with patch.object(settings, "COMMENT_SHOW_FLAGGED", False):
            self.assertEqual(
                Comment.objects.all_exclude_flagged().count(), self.increment - 1
            )
    ...
    def test_all_comments_by_object(self):
        ...
        count = Comment.objects.all_comments_by_object(
            self.post_1, include_flagged=True
        ).count()
    ...
    def test_filter_parents_by_object(self):
        ...
        # comments with flagged
        count = Comment.objects.filter_parents_by_object(
            self.post_2, include_flagged=True
        ).count()
