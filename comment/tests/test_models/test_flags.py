...
from comment.models import Flag, FlagInstance
...
class FlagInstanceManagerTest(BaseCommentFlagTest):
    def test_clean_reason_for_invalid_value(self):
        ...
        self.assertRaises(
            ValidationError, self.set_flag, self.user, self.comment, **data
        )
    ...
    def test_create_flag_twice(self):
        ...
        self.assertRaises(
            ValidationError, self.set_flag, self.user, self.comment, **self.flag_data
        )
...
class ToggleStateTest(BaseCommentFlagTest):
    ...
    def test_unflagged_state(self):
        ...
        self.assertRaises(
            ValidationError, self.flag.toggle_state, self.flag.FLAGGED, self.moderator
        )
