...
class BlockerModelTest(BaseBlockerManagerTest):
    def test_blocked_user_str(self):
        self.assertEqual(
            str(self.blocked_user_by_id), self.blocked_user_by_id.user.username
        )

    def test_blocked_email_str(self):
        self.assertEqual(
            str(self.blocked_user_by_email), self.blocked_user_by_email.email
        )
...
class BlockerManagerTest(BaseBlockerManagerTest):
    ...
    def test_is_user_blocked_for_blocked_user(self):
        self.assertTrue(
            BlockedUser.objects.is_user_blocked(user_id=self.blocked_user.id)
        )
    ...
    def test_is_user_blocked_for_unblocked_email(self):
        self.assertFalse(
            BlockedUser.objects.is_user_blocked(email="unblocked@test.com")
        )
    ...
    def test_get_blocked_user_by_user_id_when_already_exists(self):
        (
            blocked_user,
            created,
        ) = BlockedUser.objects._get_or_create_blocked_user_by_user_id(
            self.blocked_user.id
        )
    ...
    def test_get_blocked_user_by_email_when_already_exists(self):
        (
            blocked_user,
            created,
        ) = BlockedUser.objects._get_or_create_blocked_user_by_email(self.blocked_email)
        self.assertFalse(created)
    ...
    def test_create_blocked_user_by_email(self):
        (
            blocked_user,
            created,
        ) = BlockedUser.objects._get_or_create_blocked_user_by_email("test@test.com")
        self.assertTrue(created)
