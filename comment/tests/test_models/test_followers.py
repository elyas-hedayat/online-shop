...
class FollowerModelTest(BaseCommentTest):
    ...
    def setUpTestData(cls):
        ...
        cls.follower = Follower.objects.create(
            email=cls.email, username="test", content_object=cls.comment_test_follow
        )
    ...
    def test_string_value(self):
        self.assertEqual(
            str(self.follower),
            f"{str(self.comment_test_follow)} followed by {self.email}",
        )
        self.assertEqual(
            repr(self.follower),
            f"{str(self.comment_test_follow)} followed by {self.email}",
...
class FollowerManagerTest(BaseCommentTest):
    ...
    def setUpTestData(cls):
        ...
        cls.comment_test_follow = cls.create_comment(cls.content_object_1)
        cls.comment_without_email = cls.create_comment(
            cls.content_object_1, user=cls.user_without_email
        )
    ...
    def test_is_following(self):
        self.assertTrue(
            self.manager.is_following(self.follower_email, self.comment_test_follow)
        )
        self.assertFalse(
            self.manager.is_following(self.unfollower_email, self.comment_test_follow)
        )
    ...
    def test_follow_return_none_if_email_is_already_follow(self):
        ...
        self.assertIsNone(
            self.manager.follow(
                self.follower_email, "username", self.comment_test_follow
            )
        )

    def test_follow_create_follower_instance(self):
        initial_count = self.manager.count()
        follower = self.manager.follow(
            self.unfollower_email, "username", self.comment_test_follow
        )
    ...
    def test_toggle_follow_return_false_on_missing_email(self):
        ...
        result = self.manager.toggle_follow(
            email=email, username="test", model_object=self.comment_test_follow
        )
    ...
    def test_follow_parent_thread_for_comment_no_email(self):
        ...
        self.assertFalse(
            self.manager.is_following(
                self.comment_without_email.email, self.comment_without_email
            )
        )
    ...
    def test_follow_parent_thread_for_comment_child_comment(self):
        child_comment = self.create_comment(
            self.content_object_1, user=self.user_2, parent=self.comment_without_email
        )

        # the parent (thread) will not be followed on creating child comment
        self.assertFalse(
            self.manager.is_following(child_comment.email, child_comment.content_object)
        )
        # the parent comment (thread) is not followed yet
        self.assertFalse(
            self.manager.is_following(child_comment.email, self.comment_without_email)
        )
    ...
    def test_follow_parent_thread_for_comment_parent_comment(self):
        ...
        self.assertFalse(
            self.manager.is_following(
                parent_comment.email, parent_comment.content_object
            )
        )
        # parent comment is not followed yet
        self.assertFalse(
            self.manager.is_following(parent_comment.email, parent_comment)
        )
    ...
    def test_get_all_followers_for_model_object(self):
        ...
        self.assertEqual(
            ...
            list(
                self.manager.filter(
                    content_type=content_type, object_id=self.comment_test_follow.id
                )
            ),
        )
