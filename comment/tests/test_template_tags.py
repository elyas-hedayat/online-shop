...
class GetImgPathTest(BaseTemplateTagsTest):
    ...
    def test_profile_with_image(self):
        with patch(
            "comment.templatetags.comment_tags.get_profile_instance", return_value=None
        ):
            url = get_img_path(self.parent_comment_1)
            self.assertEqual(url, "/static/img/default.png")
...
class RenderContentTest(BaseTemplateTagsTest):
    ...
    @patch.object(settings, "COMMENT_ALLOW_MARKDOWN", True)
    def test_raises_improperly_configured_error_with_markdown_not_installed_and_markdown_set_to_true(
        self,
    ):
...
class GetUsernameForCommentTest(BaseTemplateTagsTest):
    ...
    def test_use_email_first_part_enabled(self):
        self.assertEqual(
            get_username_for_comment(self.comment), self.comment.user.username
        )

        # test for anonymous
        self.assertEqual(
            get_username_for_comment(self.anonymous_comment),
            self.anonymous_comment.email.split("@")[0],
        )
    ...
    def test_use_email_first_part_disabled(self):
        ...
        self.assertEqual(
            get_username_for_comment(self.anonymous_comment),
            settings.COMMENT_ANONYMOUS_USERNAME,
        )
...
class ReactionTemplateTagsTest(BaseTemplateTagsTest):
    ...
    def test_has_reacted_on_incorrect_reaction(self):
        ...
        self.assertRaises(
            TemplateSyntaxError, has_reacted, self.comment, self.user, "likes"
        )
