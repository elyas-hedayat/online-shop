...
class BaseCommentTest(TestCase, BaseInternationalizationTest):
    ...
    def setUpTestData(cls):
        ...
        cls.user_1 = User.objects.create_user(
            username="test-1", email="test-1@acme.edu", password="1234"
        )
        cls.user_2 = User.objects.create_user(
            username="test-2", email="test-2@acme.edu", password="1234"
        )
        cls.user_without_email = User.objects.create_user(
            username="no-email", password="1234"
        )
        cls.moderator = User.objects.create_user(
            username="moderator", email="test-2@acme.edu", password="1234"
        )
        moderator_group = Group.objects.filter(name="comment_moderator").first()
        moderator_group.user_set.add(cls.moderator)
        cls.admin = User.objects.create_user(
            username="admin", email="test-2@acme.edu", password="1234"
        )
        admin_group = Group.objects.filter(name="comment_admin").first()
        admin_group.user_set.add(cls.admin)
        cls.post_1 = Post.objects.create(
            author=cls.user_1, title="post 1", body="first post body"
        )
        cls.post_2 = Post.objects.create(
            author=cls.user_1, title="post 2", body="second post body"
        )
    ...
    def create_anonymous_comment(
        cls, ct_object=None, email=None, posted=None, parent=None
    ):
        if not ct_object:
    ...
    def create_reaction_instance(cls, user, comment, reaction):
        ...
        if reaction_type:
            ...
            reaction_instance = ReactionInstance.objects.create(
                user=user, reaction_type=reaction_type.value, reaction=reaction_obj
            )
    ...
    def create_flag_instance(cls, user, comment, **kwargs):
        instance = FlagInstance.objects.create(user=user, flag=comment.flag, **kwargs)
        cls.flags += 1
        return instance
    ...
    def assertQuerysetEqual(self, qs, values, transform=None, ordered=True, msg=None):
        ...
        return super().assertQuerysetEqual(
            qs, values, transform=transform, ordered=True, msg=msg
        )
...
class BaseAPITest(BaseCommentTest):
    ...
    def setUpTestData(cls):
        ...
        cls.reaction_2 = cls.create_reaction_instance(
            cls.user_1, cls.comment_5, "dislike"
        )
...
class BaseCommentManagerTest(BaseCommentTest):
    ...
    def setUpTestData(cls):
        ...
        cls.child_comment_1 = cls.create_comment(
            cls.content_object_1, parent=cls.parent_comment_1
        )
        cls.child_comment_2 = cls.create_comment(
            cls.content_object_1, parent=cls.parent_comment_2
        )
        cls.child_comment_3 = cls.create_comment(
            cls.content_object_1, parent=cls.parent_comment_2
        )

        cls.parent_comment_4 = cls.create_comment(cls.content_object_2)
        cls.parent_comment_5 = cls.create_comment(cls.content_object_2)
        cls.child_comment_4 = cls.create_comment(
            cls.content_object_2, parent=cls.parent_comment_1
        )
        cls.child_comment_5 = cls.create_comment(
            cls.content_object_2, parent=cls.parent_comment_2
        )
...
class BaseCommentViewTest(BaseCommentTest):
    ...
    def get_url(reverse_name, pk=None, data=None):
        ...
        query_string = "&".join(
            [f"{name}={quote_plus(str(value))}" for name, value in data.items()]
        )
        if query_string:
...
class BaseCommentFlagTest(BaseCommentViewTest):
    ...
    def setUpTestData(cls):
        ...
        cls.flag_instance = cls.create_flag_instance(
            cls.user_2, cls.comment_2, **cls.flag_data
        )
...
class BaseTemplateTagsTest(BaseCommentTest):
    ...
    def setUpTestData(cls):
        ...
        cls.anonymous_child_comment = cls.create_anonymous_comment(
            parent=cls.parent_comment_1
        )
...
class BaseCommentMigrationTest(TransactionTestCase):
    ...
    def force_migrate(self, migrate_to=None):
        ...
        if migrate_to is None:
            ...
            migrate_to = [
                key
                for key in self.executor.loader.graph.leaf_nodes()
                if key[0] == self.app
            ]
    ...
    def _create_data(self):
        ...
        self.post = Post.objects.create(
            author=self.user, title="post 3", body="third post body"
        )
        content_type = ContentType.objects.get(model="post")
...
class BaseBlockerManagerTest(BaseCommentTest):
    ...
    def setUpTestData(cls):
        ...
        cls.blocked_user = User.objects.create_user(
            username="blocked user", password="1234"
        )
        cls.unblocked_user = User.objects.create_user(
            username="unblocked user", password="1234"
        )
    ...
    def create_blocked_email(email):
        blocked_email, _ = BlockedUser.objects._get_or_create_blocked_user_by_email(
            email
        )
    ...
    def create_blocked_user(user_id):
        blocked_user, _ = BlockedUser.objects._get_or_create_blocked_user_by_user_id(
            user_id
        )
        return blocked_user
